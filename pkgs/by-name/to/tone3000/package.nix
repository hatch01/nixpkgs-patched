...
let
  ...
  # The project's git submodules, fetched separately (the GitHub source
  # archive doesn't include them).
  namCore = fetchFromGitHub {
    owner = "sdatkinson";
    repo = "NeuralAmpModelerCore";
    rev = "1f42f88535884450104b8711d7595019afa0495b"; # from .gitmodules / gitlink
    hash = "sha256-IHBllDeZqiwwYAgKD+O22T6rmlXihNLfwD2GzosqV9M=";
    fetchSubmodules = true; # Dependencies/eigen and Dependencies/AudioDSPTools
  };

  audioDspTools = fetchFromGitHub {
    owner = "sdatkinson";
    repo = "AudioDSPTools";
    rev = "0827c6c2fc0deced568536142ea86f189e0b98a1"; # from .gitmodules / gitlink
    hash = "sha256-5kbG9K2bwk0rbfQ72uMfhbP64dNT6S7lCfp+wStkmeM=";
  };
  ...
  src = fetchFromGitHub {
    ...
    tag = "v${version}";
    hash = "sha256-KfhMVC4IDoKD/luEL4d/CwFi8XR4YZLRTqWxmQlKV04=";
  };
...
stdenv.mkDerivation (finalAttrs: {
  ...
  postUnpack = ''
    ...
    rm -rf "$sourceRoot/plugin/NeuralAmpModelerCore" "$sourceRoot/plugin/AudioDSPTools"
    cp -R --no-preserve=mode,ownership ${namCore} "$sourceRoot/plugin/NeuralAmpModelerCore"
    cp -R --no-preserve=mode,ownership ${audioDspTools} "$sourceRoot/plugin/AudioDSPTools"
  '';
